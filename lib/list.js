...
class List {
   ...
   */
	...
	/**
   * Find a category by case-insensitive name across all groups in this list.
   * Returns the first match. Useful when callers identify categories by name.
   * @param {string} name
   * @return {{group: object, category: object}|null}
   */
	findCategoryByName(name) {
		...
		for (const group of this.categoryGroups) {
			const category = group.categories.find(c => (c.name || '').toLowerCase() === lower);
			if (category) {
				return {group, category};
			}
		}
